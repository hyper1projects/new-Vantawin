...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const successUrl = success_url || `${Deno.env.get('SITE_URL')}/games`
        const cancelUrl = cancel_url || `${Deno.env.get('SITE_URL')}/wallet`
        ...
        const response = await fetch('https://api-sandbox.nowpayments.io/v1/invoice', {
            ...
            body: JSON.stringify({
                ...
                success_url: successUrl, // Redirect back to games
