-- Migration: Restrict Concurrent Pool Joins
-- Purpose: Prevent users from joining a new pool if they are already in an 'ongoing' or 'upcoming' one.
...
BEGIN
    ...
    -- [NEW] 2.5 Strict Single-Entry Policy
    -- Check if user is already in ANY active pool (ongoing or upcoming)
    -- We exclude the current pool ID to allow idempotent retries if needed (though existing check below covers that)
    IF EXISTS (
        SELECT 1 
        FROM public.tournament_entries e
        JOIN public.pools p ON e.pool_id = p.id
        WHERE e.user_id = p_user_id
        AND p.status IN ('ongoing', 'upcoming')
        AND p.id != p_pool_id -- Ignore self if re-joining
    ) THEN
        RAISE EXCEPTION 'You are already active in another tournament. Please finish your current tournament before joining a new one.';
    END IF;
    ...
    -- 4. Check for Existing Entry (Idempotency)
