...
const OddsButton: React.FC<OddsButtonProps> = ({ label, onClick, isSelected, className }) => (
    <button
        onClick={onClick}
        className={cn(
            "bg-[#0B295B] text-white border border-gray-600 h-8 px-3 text-sm rounded-lg transition-colors shadow-inner font-semibold flex items-center justify-center", // Changed rounded to rounded-lg for 8px corner smoothing
