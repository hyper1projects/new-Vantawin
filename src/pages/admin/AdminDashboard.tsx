...
import { Loader2, RefreshCw, Trophy, AlertTriangle, CheckCircle, XCircle } from 'lucide-react';
        ...
        <div className="min-h-screen bg-vanta-dark text-white p-6 pb-20 md:pb-6 ml-0 md:ml-64">
            ...
            <div className="flex gap-4 mb-6 border-b border-white/10">
                <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'matches'
...
function AdminMatches() {
    ...
    const fetchMatches = async () => {
        setLoading(true);
        // Fetch recent matches or specific ones
        const { data, error } = await supabase
            .from('matches')
            .select('*, home_team, away_team') // Ensure joins if needed/or assume jsonb
                        ...
                        <div key={match.id} className="bg-white/5 p-4 rounded-xl flex justify-between items-center border border-white/5 hover:border-white/10 transition-all">
                            <div>
                                ...
                                <div className="font-semibold text-lg flex items-center gap-2">
                                    {/* Handle both JSONB and joined relation structures if needed. 
                                        Assuming fetch-odds saves names in jsonb OR we rely on fetch join?
                                        Actually fetch-odds logic saves `home_team` as jsonb. Good.
                                    */}
